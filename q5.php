<h1>Vie associative, culturelle et sportive</h1>
<img src="img.jpg" class="img-polaroid" alt="Laurence Maroye" height="250" width="250" style="display: block; margin-left: auto; margin-right: auto;"/> <!-- TODO: Mettre la bonne image et adapter la taille -->
<p>Combien de Cercles et Associations sont actuellement reconnus par l'Université?</p>
<form class="navbar-form pull-left">
	<label class="radio">
	  <input type="radio" name="optionsq5" id="optionsRadios1" value="10">
	  Une dizaine.
	</label>
	<label class="radio">
	  <input type="radio" name="optionsq5" id="optionsRadios2" value="60">
	  Une soixantaine.
	</label>
	<label class="radio">
	  <input type="radio" name="optionsq5" id="optionsRadios3" value="100">
	  Une centaine.
	</label>
	<button type="submit" class="btn">Valider</button> <!-- TODO : Adapter le submit -->
</form>