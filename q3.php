<h1>Mobilité à l'ULB</h1>
<img src="img.jpg" class="img-polaroid" alt="Laurence Maroye" height="250" width="250" style="display: block; margin-left: auto; margin-right: auto;"/> <!-- TODO: Mettre la bonne image et adapter la taille -->
<p>Combien de parkings vélo (bornes Villo non comprises) peux-tu trouver sur le Campus du Solbosch ?</p>
<form method="post" name="reponse3" action="index.php?r3" class="navbar-form pull-left">
  <label class="radio">
    <input type="radio" name="reponse3" id="optionsRadios1" value="2">
    2
  </label>
  <label class="radio">
    <input type="radio" name="reponse3" id="optionsRadios2" value="4">
    4
  </label>
  <label class="radio">
    <input type="radio" name="reponse3" id="optionsRadios3" value="8">
    8
  </label>
  <button type="submit" class="btn">Valider</button> <!-- TODO : Adapter le submit -->
</form>